...
- **确认前端入口**：公开产品首页是 `/`，普通用户工作台是 `/login`，项目状态页是 `/health`，后台入口是 `/admin`；这些入口的使用说明在 [`../README.md`](../README.md) 和 [`API.md`](./API.md) 里维护。
...
| HTTP API / SSE / auth / frontend routes | [`API.md`](./API.md) | 按真实请求顺序查接口，并说明 `/`、`/login`、`/health`、`/admin` 入口边界。 |
