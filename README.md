...
> 面向美国非移民签证场景的 AI Agent 面签工作台：用多轮追问、多模态案例理解、证据图谱和风险报告，帮助申请人更早发现 DS-160 叙事中的漏洞、薄弱证明点和不一致。
...
- 🧭 识别当前回答中的薄弱证明点、材料冲突、不完整解释和高风险信号
- 📊 生成面向用户的准备建议和面向内部调试的运行报告
- 🧪 验证 LangGraph runtime、Case Memory、材料理解、RAG 检索、Governor 护栏和前端 Case Board 体验
...
LangGraph DS160TurnGraph
        │
        ├── GraphCaseStateBuilder
        ├── Typed LLM Nodes via Pydantic AI
...
| `app/services/` | 业务编排层，承载 LangGraph runtime、Case Memory、材料理解、报告生成和状态同步 |
...
5. `GraphCaseStateBuilder` 把 Case Memory、最近 turn、材料证据、RAG 引用和旧兼容字段组装成 graph state。
6. LangGraph 中的 typed LLM node 选择下一问；Governor / guard 只负责高风险、拒签和 grounding 边界。
...
当前主流程由 LangGraph 编排。Pydantic AI 只作为 typed LLM call 层；未来如果试验 OpenAI Agents SDK，也只能藏在单个 `LLMNodeRunner` 节点后面，不能接管会话状态、handoff 或用户可见主回复。
